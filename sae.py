...
class MatchingPursuitSAE(nn.Module):
    def __init__(
        self,
        d_model,
        n_latents,
    ...
    def step(self, x, return_metrics=False):
        ...
        if return_metrics:
            tot_var = ((x - x.mean(dim=0, keepdim=True)) ** 2).sum(dim=-1).mean()
            err = ((x_hat - x) ** 2).sum(dim=-1).mean()
            result["fvu"] = err / tot_var
...
class BatchTopKSAE(nn.Module):
    def __init__(
        ...
        self.W_enc = nn.Parameter(torch.randn(n_latents, d_model))
        self.b_enc = nn.Parameter(torch.zeros(n_latents))
        self.W_dec = nn.Parameter(
            (self.W_enc.data / self.W_enc.data.norm(dim=0, keepdim=True)).T
        )
    ...
    def step(self, x, return_metrics=False):
        ...
        values, indices = torch.topk(
            torch.relu(Wtr_b.flatten()), dim=0, k=int(self.target_l0 * batch_size) + 1
        )

        z = (
            torch.zeros(batch_size * self.n_latents, device=self.device)
            .scatter(0, indices, values)
            .reshape(batch_size, self.n_latents)
        )
        if torch.isnan(self.threshold):
            self.threshold = values[-1].detach()
        else:
            self.threshold = (
